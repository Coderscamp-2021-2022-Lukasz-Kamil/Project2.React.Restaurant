...
const DishForm = ()  => {
  ...
  const updateDish = () => {
    if (!name || !cost || !imgLink || !description || !ingredients || (category==="Category")) {
      ...
      editDish(dishId, name, cost, category, imgLink, description, ingredients, spiciness, isVege);
      setMessage('Dish was edited!'); 
  ...
  return (
    <Container className={`${styles.dishFormContainer}`}>
      ...
      >
        ...
        {isValid ? message : <p style={{color: "red"}}>Some fields are empty!</p>}
        <FormGroup
        ...
        >
          ...
          <Button
            onClick={editMode ? updateDish : createDish}
